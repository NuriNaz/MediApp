...
const PunchOutTime = () => {
    ...
    const getAttendanceDetails = async () => {
        ...
        try {
            ...
            if (result.status === 200) {
                ...
                setSelectedOption2(data.break_availed);
                setDetails({
    ...
    function secondsToTime(seconds) {
        ...
        const formattedTime = `${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(remainingSeconds).padStart(
            2,
            '0'
        )}`;
    ...
    const paidBreak = secondsToTime(selectedOption2);
